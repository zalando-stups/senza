...
from .exceptions import ELBNotFound, HostedZoneNotFound, InvalidState, RecordNotFound
...
class Route53HostedZone:
    ...
    def from_boto_dict(cls, hosted_zone_dict: Dict[str, Any]) -> "Route53HostedZone":
        ...
        return cls(id, name, caller_reference, config, resource_record_set_count)
    ...
    def change(
        ...
        for action, record in changes:
            change = {"Action": action.upper(), "ResourceRecordSet": record.boto_dict}
            change_batch["Changes"].append(change)

        if change_batch["Changes"]:
            client.change_resource_record_sets(
                HostedZoneId=self.id, ChangeBatch=change_batch
            )

        return change_batch

    def create(
        self, changed_records: Iterable["Route53Record"], comment: Optional[str] = None
    ) -> Dict[str, Any]:
        """
...
class Route53Record:
    ...
    def boto_dict(self):
        ...
        boto_dict = {"Name": self.name, "Type": self.type.value}
    ...
    def to_alias(self, target_type: RecordType = RecordType.A) -> "Route53Record":
        """
        ...
        if self.type == RecordType.CNAME:
            ...
            raise NotImplementedError(
                "Conversion of {} records to Alias is "
...
class Route53:
    ...
    def get_hosted_zones(
        domain_name: Optional[str] = None, id: Optional[str] = None
    ) -> Iterator[Route53HostedZone]:
        """
    ...
    @classmethod
    def get_records(cls, *, name: Optional[str] = None) -> Iterator[Route53Record]:
        client = BotoClientProxy("route53")
        ...
        for zone in cls.get_hosted_zones():
            ...
            for resource in resources:
                record = Route53Record.from_boto_dict(resource, hosted_zone=zone)
...
def convert_cname_records_to_alias(domain_name: str):
    ...
    converted_records = defaultdict(lambda: {"delete": [], "upsert": []})
    ...
    if converted_records:
        for hosted_zone, records in converted_records.items():
            ...
            msg = (
                ...
                "Alias records".format(
                    name=domain_name, hz=hosted_zone.name, n=len(records["upsert"]), s=s
                )
            )
            ...
            if not interactive_terminal or confirm(msg):
                ...
                hosted_zone.upsert(
                    records["upsert"], comment="Convert non alias records"
                )
                print("  Inserted alias record{s}".format(s=s))
            else:
                raise InvalidState(
                    "Can't change domains because there are " "non A Type records."
                )
